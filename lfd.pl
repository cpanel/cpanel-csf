...
our (
    $abuseip,             $accept,             $apache401timeout, $apache403timeout,
...
our (
    @cccidrs,       @cidrs,          @faststart4, @faststart4nat, @faststart6,
...
sub ipblock {
    ...
    my @report;
    $report[0] = $ip;
    if   ($port) { $report[1] = "$port" }
    else         { $report[1] = "*" }
    if   ($perm) { $report[2] = "1" }
    else         { $report[2] = "0" }
    if   ($inout) { $report[3] = "$inout" }
    else          { $report[3] = "inout" }
    if   ($timeout) { $report[4] = "$timeout" }
    else            { $report[4] = "0" }
    if   ($message) { $report[5] = "$message" }
    else            { $report[5] = " " }
    if   ($logs) { $report[6] = "$logs" }
    else         { $report[6] = " " }
...
sub stats_report {
    ...
    unless ($childpid) {
        ...
        if   ($perm)                                  { $permdate = $time; $permcount++ }
        else                                          { $tempdate = $time; $tempcount++ }
...
sub domessenger {
    ...
    foreach my $port ( split( /\,/, $ports ) ) {
        if ( $httpsin{$port} ) {
            if ( $httpsports eq "" ) { $httpsports = "$port" }
            else                     { $httpsports .= ",$port" }
        }
        if ( $htmlin{$port} ) {
            if ( $htmlports eq "" ) { $htmlports = "$port" }
            else                    { $htmlports .= ",$port" }
        }
        if ( $textin{$port} ) {
            if ( $textports eq "" ) { $textports = "$port" }
            else                    { $textports .= ",$port" }
        }
    }
