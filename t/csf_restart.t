...
$slurp_mock->redefine('slurpee', sub {
    my ($file, %opts) = @_;
    $file =~ s{^/etc/csf/}{/opt/repos/app-csf/tmp/};
...
subtest 'modulino pattern implemented correctly' => sub {
    ...
    like($source, qr/__PACKAGE__->run\(\) unless caller;/, 
         'Has modulino pattern: __PACKAGE__->run() unless caller');
    
    like($source, qr/sub run \{/, 
    ...
    like($run_sub, qr/\$version = version\(\);/, 
