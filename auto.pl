...
open( my $version_fh, "<", "/etc/csf/version.txt" );
flock( $version_fh, LOCK_SH );
...
if ( -e "/proc/vz/veinfo" ) {
    ...
    open( my $in_fh, "<", "/proc/self/status" );
    flock( $in_fh, LOCK_SH );
    while ( my $line = <$in_fh> ) {
        chomp $line;
        if ( $line =~ /^envID:\s*(\d+)\s*$/ ) {
...
if ( checkversion("7.72") and -e "/etc/csf/csf.pignore" and !-e "/var/lib/csf/auto772" ) {
    ...
    close($in_fh);
    open( my $out_fh, ">", "/var/lib/csf/auto772" );
...
if ( checkversion("8.04") and -e "/etc/csf/csf.pignore" and !-e "/var/lib/csf/auto804" ) {
    ...
    close($in_fh);
    open( my $out_fh, ">", "/var/lib/csf/auto804" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("8.06") and -e "/etc/csf/csf.pignore" and !-e "/var/lib/csf/auto806" ) {
    ...
    close($in_fh);
    open( my $out_fh, ">", "/var/lib/csf/auto806" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("8.13") and -e "/etc/csf/csf.pignore" and !-e "/var/lib/csf/auto813" ) {
    ...
    close($in_fh);
    open( my $out_fh, ">", "/var/lib/csf/auto813" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("8.26") and -e "/etc/csf/csf.pignore" and !-e "/var/lib/csf/auto826" ) {
    ...
    close($in_fh);
    open( my $out_fh, ">", "/var/lib/csf/auto826" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("8.27") and -e "/etc/csf/csf.pignore" and !-e "/var/lib/csf/auto827" ) {
    ...
    close($in_fh);
    open( my $out_fh, ">", "/var/lib/csf/auto827" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("9.00") and !-e "/var/lib/csf/auto900" ) {
    ...
    my %allow = ( 'Include /etc/csf/cpanel.comodo.allow' => 1 );
    sysopen( $in_fh, "/etc/csf/csf.allow", O_RDWR | O_CREAT );
    flock( $in_fh, LOCK_EX );
    @data = <$in_fh>;
    chomp @data;
    seek( $in_fh, 0, 0 );
    ...
    my %ignore = ( 'Include /etc/csf/cpanel.comodo.ignore' => 1 );
    sysopen( $in_fh, "/etc/csf/csf.ignore", O_RDWR | O_CREAT );
    flock( $in_fh, LOCK_EX );
    @data = <$in_fh>;
    chomp @data;
    seek( $in_fh, 0, 0 );
    ...
    open( my $out_fh, ">", "/var/lib/csf/auto900" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("9.12") and -e "/etc/csf/csf.pignore" and !-e "/var/lib/csf/auto912" ) {
    ...
    close($in_fh);
    open( my $out_fh, ">", "/var/lib/csf/auto912" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("10.02") and -e "/etc/csf/csf.pignore" and !-e "/var/lib/csf/auto1002" ) {
    ...
    close($in_fh);
    open( my $out_fh, ">", "/var/lib/csf/auto1002" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("10.06") and -e "/etc/csf/csf.pignore" and !-e "/var/lib/csf/auto1006" ) {
    ...
    close($in_fh);
    open( my $out_fh, ">", "/var/lib/csf/auto1006" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("10.08") and !-e "/var/lib/csf/auto1008" ) {
    ...
    my %ignore = ( 'Include /etc/csf/cpanel.ignore' => 1 );
    sysopen( $in_fh, "/etc/csf/csf.ignore", O_RDWR | O_CREAT );
    flock( $in_fh, LOCK_EX );
    @data = <$in_fh>;
    chomp @data;
    ...
    open( my $out_fh, ">", "/var/lib/csf/auto1008" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("10.11") and !-e "/var/lib/csf/auto1011" ) {
    ...
    open( my $out_fh, ">", "/var/lib/csf/auto1011" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("10.23") and !-e "/var/lib/csf/auto1023" ) {
    if ( -e "/etc/csf/csf.blocklists" ) {
        ...
        unless ($SPAMDROPV6) {
            ...
            print $in_fh "# Details: http://www.spamhaus.org/drop/\n";
            print $in_fh "#SPAMDROPV6|86400|0|https://www.spamhaus.org/drop/dropv6.txt\n";
        }
        unless ($STOPFORUMSPAMV6) {
            print $in_fh "\n# Stop Forum Spam IPv6\n";
            print $in_fh "# Details: http://www.stopforumspam.com/downloads/\n";
            print $in_fh "# Many of the lists available contain a vast number of IP addresses so special\n";
            print $in_fh "# care needs to be made when selecting from their lists\n";
            print $in_fh "#STOPFORUMSPAMV6|86400|0|http://www.stopforumspam.com/downloads/listed_ip_1_ipv6.zip\n";
        }
        close($in_fh);
    }

    open( my $out_fh, ">", "/var/lib/csf/auto1023" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("11.07") and -e "/etc/csf/csf.pignore" and !-e "/var/lib/csf/auto1107" ) {
    ...
    close($in_fh);
    open( my $out_fh, ">", "/var/lib/csf/auto1107" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("12.02") and !-e "/var/lib/csf/auto1202" ) {
    ...
    open( my $out_fh, ">", "/var/lib/csf/auto1202" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("12.05") and !-e "/var/lib/csf/auto1205" ) {
    ...
    open( my $out_fh, ">", "/var/lib/csf/auto1205" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("12.07") and -e "/etc/csf/csf.pignore" and !-e "/var/lib/csf/auto1207" ) {
    ...
    close($in_fh);
    open( my $out_fh, ">", "/var/lib/csf/auto1207" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("12.10") and -e "/etc/csf/csf.fignore" and !-e "/var/lib/csf/auto1210" ) {
    ...
    close($in_fh);
    open( my $out_fh, ">", "/var/lib/csf/auto1210" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("14.01") and -e "/etc/csf/csf.pignore" and !-e "/var/lib/csf/auto1401" ) {
    ...
    close($in_fh);
    open( my $out_fh, ">", "/var/lib/csf/auto1401" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("14.02") and -e "/etc/csf/csf.pignore" and !-e "/var/lib/csf/auto1402" ) {
    ...
    close($in_fh);
    open( my $out_fh, ">", "/var/lib/csf/auto1402" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("14.03") and !-e "/var/lib/csf/auto1403" ) {
    ...
    open( my $out_fh, ">", "/var/lib/csf/auto1403" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("14.05") and -e "/etc/csf/csf.pignore" and !-e "/var/lib/csf/auto1405" ) {
    ...
    close($in_fh);
    open( my $out_fh, ">", "/var/lib/csf/auto1405" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("14.06") and -e "/etc/csf/csf.pignore" and !-e "/var/lib/csf/auto1406" ) {
    ...
    close($in_fh);
    open( my $out_fh, ">", "/var/lib/csf/auto1406" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
if ( checkversion("14.18") and -e "/etc/csf/csf.pignore" and !-e "/var/lib/csf/auto1418" ) {
    ...
    close($in_fh);
    open( my $out_fh, ">", "/var/lib/csf/auto1418" );
    flock( $out_fh, LOCK_EX );
    print $out_fh time;
    close($out_fh);
}
...
if ( -e "/usr/local/csf/bin/regex.custom.pm" ) {
    sysopen( my $in_fh, "/usr/local/csf/bin/regex.custom.pm", O_RDWR | O_CREAT );
    flock( $in_fh, LOCK_EX );
    my @data = <$in_fh>;
    chomp @data;
    seek( $in_fh, 0, 0 );
...
if ( -e "/var/lib/csf/csf.tempban" ) {
    sysopen( my $in_fh, "/var/lib/csf/csf.tempban", O_RDWR | O_CREAT );
    flock( $in_fh, LOCK_EX );
    my @data = <$in_fh>;
    chomp @data;
    seek( $in_fh, 0, 0 );
...
if ( -e "/var/lib/csf/csf.tempallow" ) {
    sysopen( my $in_fh, "/var/lib/csf/csf.tempallow", O_RDWR | O_CREAT );
    flock( $in_fh, LOCK_EX );
    my @data = <$in_fh>;
    chomp @data;
    seek( $in_fh, 0, 0 );
...
if ( -e "/usr/local/csf/tpl/reselleralert.txt" ) {
    sysopen( my $in_fh, "/usr/local/csf/tpl/reselleralert.txt", O_RDWR | O_CREAT );
    flock( $in_fh, LOCK_EX );
    my @data = <$in_fh>;
    chomp @data;
    seek( $in_fh, 0, 0 );
    ...
    unless ($text) { print $in_fh "\n[text]\n" }
    close($in_fh);
}

open( my $in_fh, "<", "/etc/chkserv.d/chkservd.conf" );
flock( $in_fh, LOCK_SH );
my @chkservd = <$in_fh>;
close($in_fh);
chomp @chkservd;
...
if ( not grep { $_ =~ /^lfd/ } @chkservd ) {
    open( my $out_fh, ">", "/etc/chkserv.d/lfd" );
    flock( $out_fh, LOCK_EX );
    print $out_fh "service[lfd]=x,x,x,service lfd restart,lfd,root\n";
    close($out_fh);

    sysopen( my $out_fh2, "/etc/chkserv.d/chkservd.conf", O_WRONLY | O_CREAT );
    flock( $out_fh2, LOCK_EX );
    foreach my $line (@chkservd) { print $out_fh2 "$line\n" }
    print $out_fh2 "lfd:1\n";
    close $out_fh2;
}
...
if ( $config{TESTING} ) {
    ...
    if ( $sshport ne "22" ) {
        ...
        foreach my $line (@config) {
            if ( $line =~ /^TCP6_IN/ ) {
                print $out_fh "TCP6_IN = \"$config{TCP6_IN}\"\n";
                print "\n*** SSH port $sshport added to the TCP6_IN port list\n\n";
            }
            elsif ( $line =~ /^TCP_IN/ ) {
                print $out_fh "TCP_IN = \"$config{TCP_IN}\"\n";
    ...
    if ( -e $config{IP6TABLES} and !$vps ) {
        ...
        if ( grep { $_ =~ /\s*inet6/ } @ifconfig ) {
            ...
            foreach my $line (@config) {
                if ( $line =~ /^IPV6 =/ ) {
                    print $out_fh "IPV6 = \"$config{IPV6}\"\n";
                    print "\n*** IPV6 Enabled\n\n";
                }
                elsif ( $line =~ /^IPV6_SPI =/ ) {
                    print $out_fh "IPV6_SPI = \"$config{IPV6_SPI}\"\n";
    ...
    if ( $exim ne "0" ) {
        ...
        foreach my $line (@config) {
            if ( $line =~ /^TCP_IN =/ ) {
                ...
                print $out_fh "SMTP_PORTS = \"$config{SMTP_PORTS}\"\n";
...
sub checkversion {
    ...
    if   ( ( $oldmaj < $maj ) or ( $oldmaj == $maj and $oldmin < $min ) ) { return 1 }
    else                                                                  { return 0 }
}
###############################################################################
